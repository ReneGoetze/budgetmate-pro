...
function currentMonthTotal(exp){return filterSelectedMonth(exp).reduce((s,e)=>s+Number(e.amount||0),0);}
...
function buildYearlyData(expenses,year,settings){
  ...
  for(let m=0;m<12;m++){
    ...
    const sum=expenses.filter(e=>(e.date||'').startsWith(ym)).reduce((s,e)=>s+Number(e.amount||0),0);
    labels.push(MONTHS[m]);values.push(sum);totalYear+=sum;
    ...
    budgets.push(limit);
    if(limit!=null){
      const d=sum-limit;diffs.push(d);
      colors.push(sum<=limit?COLOR_GREEN:COLOR_RED);
    }else{diffs.push(null);colors.push(COLOR_NEUTRAL);}
  }
...
function buildWeeklyData(expenses){
  ...
  for(let i=11;i>=0;i--){
    ...
    labels.push(label);values.push(sum);total+=sum;
...
function updateMonthSummary(monthExp,settings){
  ...
  if(budget!=null){
    ...
    if(diff>0){txt+=' • '+diff.toFixed(2)+' € OVER BUDGET';el.className='hint summary-warn';}
    else if(diff<0){txt+=' • '+Math.abs(diff).toFixed(2)+' € UNDER BUDGET';el.className='hint summary-ok';}
    else{txt+=' • exactly on budget';el.className='hint';}
...
function updateMonthChangeAndProjection(monthExp,settings){
  ...
  if(changeEl){
    ...
    const selSum=all.filter(e=>(e.date||'').startsWith(selKey)).reduce((s,e)=>s+Number(e.amount||0),0);
    const prevSum=all.filter(e=>(e.date||'').startsWith(prevKey)).reduce((s,e)=>s+Number(e.amount||0),0);
    if(prevSum===0){changeEl.textContent='Change vs previous month: no previous month data.';changeEl.className='hint';}
    else{
      const diff=selSum-prevSum;
      const arrow=diff>0?'↑':(diff<0?'↓':'→');const abs=Math.abs(diff).toFixed(2);
  ...
  if(projEl){
    const now=new Date();const sel=getSelectedMonthDate();
...
function populateMonthYearSelectors(){
  ...
  MONTHS.forEach((name,idx)=>{
    const opt=document.createElement('option');opt.value=idx;opt.textContent=name;mSel.appendChild(opt);
  });
...
function renderCharts(){
  ...
  m.values.forEach(v=>{running+=v;cumValues.push(running);});
  ...
  if(yearSumEl){
    yearSumEl.textContent='Year '+year+': Total '+yData.totalYear.toFixed(2)+' € • Avg/month '+yData.avgMonth.toFixed(2)+
  ...
  if(yearTableEl){
    ...
    for(let i=0;i<12;i++){
      const label=yData.labels[i];const sum=yData.values[i];
      if(sum===0)continue;
      const budget=yData.budgets[i];const diff=yData.diffs[i];
      let status='-',cls='';
    ...
    html+='</tbody></table>';yearTableEl.innerHTML=html;
  }
  ...
  if(weekSummaryEl){
    weekSummaryEl.textContent='Last 12 weeks: Total '+wData.total.toFixed(2)+' € • Avg/week '+wData.avgWeek.toFixed(2)+
