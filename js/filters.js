...
function getFilteredExpenses(){
  ...
  if(currentFilter.period==='day')exp=filterCurrentDay(exp);
  else if(currentFilter.period==='week')exp=filterCurrentWeek(exp);
  else if(currentFilter.period==='month')exp=filterSelectedMonth(exp);
...
function renderExpenseTable(){
  ...
  exp.forEach(e=>{
    ...
    const dTd=document.createElement('td');dTd.textContent=e.date||'-';
    const cTd=document.createElement('td');cTd.textContent=e.category||'-';
    const aTd=document.createElement('td');aTd.textContent=Number(e.amount||0).toFixed(2);
    const delTd=document.createElement('td');
    const btn=document.createElement('button');btn.type='button';btn.textContent='Del';btn.className='del-btn';
    btn.addEventListener('click',()=>{
      if(!confirm('Do you really want to delete?'))return;
      deleteExpense(e.id);renderExpenseTable();renderCharts();renderBudgetInfo();
    });
    delTd.appendChild(btn);
    tr.appendChild(dTd);tr.appendChild(cTd);tr.appendChild(aTd);tr.appendChild(delTd);
  ...
  }else bar.className='summary-bar';
  bar.textContent=txt;
}
...
function openFilterModal(){
  ...
  if(cSel){
    ...
    cats.forEach(cat=>{
      const opt=document.createElement('option');opt.value=cat;opt.textContent=cat;cSel.appendChild(opt);
    });
