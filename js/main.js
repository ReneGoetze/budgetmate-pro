...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  if(form){
    form.addEventListener('submit',e=>{
      ...
      if(!date||!amount||!category){alert('Please enter date, amount and category.');return;}
  ...
  if(darkBtn)darkBtn.addEventListener('click',()=>{toggleDarkMode();renderCharts();});
  ...
  if(delAllBtn)delAllBtn.addEventListener('click',()=>{
    ...
    saveExpenses([]);renderExpenseTable();renderCharts();renderBudgetInfo();
  });
  ...
  if(thDate)thDate.addEventListener('click',()=>{
    currentFilter.period='day';currentFilter.from=null;currentFilter.to=null;
    renderExpenseTable();
  });
